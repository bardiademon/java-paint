...
public final class FText extends JFrame {
     ...
     */
    public FText(final On on , final Text text) {
        this.on = on;
        this.text = text;
        if (this.on == null) dispose();
            ...
            setVisible(true);
            on();
            if ((strText = this.text.getText()) != null) {
                txtText.setText(strText);
                sArc.setValue(this.text.getArc().width);
//                on.onChangedArc (sArc.getValue ());
                jpBackgroundColor.setBackground(this.text.getBackgroundColor());
                jpTextColor.setBackground(this.text.getColor());

                color = this.text.getColor();
                backgroundColor = this.text.getBackgroundColor();

                this.on.onChangedTextColor(color);
                this.on.onChangedBackgroundColor(backgroundColor);
            }
        }
    }
